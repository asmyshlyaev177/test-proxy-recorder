...
//
// READMEs put the locale in a filename and docs pages put it in a directory;
// the marketing homepage has neither, because its text is not a document. Its
// words live in packages/landing/src/i18n/home/en.ts as one typed object, and
// each language is that same object with translated values.
//
...
const MODULES = [{ source: 'packages/landing/src/i18n/home/en.ts', name: 'homepage copy' }];
...
function header(locale, blob, status) {
  return [
    `// ${locale.english} (${locale.code}) homepage copy.`,
    '//',
