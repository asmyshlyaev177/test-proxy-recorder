...
:::note[Cómo se enmascaran los archivos `.har`]
Los archivos `.har` los escribe el `routeFromHAR` de Playwright, no el proxy, así que se enmascaran en una pasada aparte. `playwrightProxy.teardown()` reescribe cada `.har` del directorio de grabaciones usando la **misma configuración de enmascaramiento** que el proxy (las cabeceras, `allowCookies` y `bodyPatterns` se aplican, tanto a las cabeceras como a los arrays `cookies` parseados). Esto se ejecuta desde tu **`globalTeardown`** de Playwright — así que el enmascaramiento de HAR requiere un `globalTeardown` que llame a `playwrightProxy.teardown()` (la [configuración recomendada](/es/docs/integrations/playwright/#global-teardown-recommended), generada por `init`).

No puede ejecutarse por prueba: Playwright vuelca un HAR cuando su contexto se cierra pero no espera a los handlers de cierre, así que enmascarar ahí compite con la salida del proceso y puede truncar el archivo. El teardown obtiene la configuración de `/__control` (el proxy debe estar en ejecución; si es inalcanzable se aplican igualmente los valores por defecto de cabeceras integrados), solo reescribe los archivos que realmente cambió y deja intactos los cuerpos de respuesta en base64. Por defensa en profundidad, graba aun así con credenciales de test de corta vida y revisa los HAR antes de hacer commit — mira el patrón de auth recomendado más abajo.
:::

## Patrón de autenticación recomendado

Para mantener el flujo de login y las credenciales totalmente fuera de las grabaciones, ejecuta la autenticación en un **setup project** de Playwright con el proxy en modo `transparent`, persiste el `storageState` en un `auth-state.json` **gitignorado** y reúsalo en tus pruebas. Las peticiones grabadas entonces solo llevan las cabeceras de sesión (enmascaradas), nunca el login.
...
## Ajustar qué se enmascara

Las cabeceras por defecto siempre se aplican (mientras el enmascaramiento está activado); puedes añadir a ellas.

### Flags de CLI

- `--no-redact` — desactiva el enmascaramiento de secretos (activado por defecto).
- `--redact` — activa el enmascaramiento de secretos; solo se necesita para reactivarlo cuando la configuración pone `redaction: false`.
- `--redact-headers <names>` — nombres de cabeceras extra a enmascarar, separados por comas (fusionados con los valores por defecto).
- `--redact-body <patterns>` — patrones regex separados por comas a enmascarar de los cuerpos de petición/respuesta.
- `--allow-headers <names>` — nombres de cabeceras separados por comas a eximir del enmascaramiento (por ejemplo `set-cookie`).
- `--allow-cookies <names>` — nombres de cookies separados por comas a mantener sin enmascarar dentro de `Cookie`/`Set-Cookie`.
...
`redactSession(session, config)` también se exporta si quieres enmascarar grabaciones existentes tú mismo.
