...
description: Поместите опции test-proxy-recorder — цель, порт, регулярные выражения для маскирования, темп WebSocket — в автоматически обнаруживаемый файл конфигурации вместо флагов CLI.
i18nSource: docs/guides/config.md
i18nSourceBlob: d633335aa6541254ec9f1af34ca98d1ee4b6d758
---

Для всего, что выходит за рамки пары флагов — особенно регулярных выражений для маскирования тела — поместите опции в файл конфигурации. Прокси автоматически обнаруживает `test-proxy-recorder.config.{ts,js,mjs,cjs}` в текущем каталоге, либо передайте `--config <path>`, чтобы указать его явно. Файлы `.ts` работают из коробки.
