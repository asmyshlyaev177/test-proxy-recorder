 ...
 * (react-horizontal-scrolling-menu.dev, asmyshlyaev177.dev) so all of them
 * negotiate alike. The token lists are cross-checked against
 ...
 * link would lose its preview card. Headless Chrome is here so this site's own
 * Lighthouse runs keep measuring the page people actually get.
 *
 ...
 * cache is unbounded.
 *
 * The `.md` mirror URLs are a different case and are cached: they have one
 * representation, so there is no second variant to hand to the wrong caller.
 *
 ...
 * A crawler that skipped robots.txt has no other way to learn the terms.
 */
