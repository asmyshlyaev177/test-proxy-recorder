...
// Proxies to the mock backend on :3002, listens on :8100, and writes recordings
// to ./e2e/recordings — plus an extra redacted header to demonstrate the
// redaction config end-to-end.
export default defineConfig({
  target: 'http://localhost:3002',
  port: 8100,
  recordingsDir: './e2e/recordings',
  // Redaction is on by default; this object customizes it. Pass `false` to disable.
  redaction: {
