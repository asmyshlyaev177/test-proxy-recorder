...
// fetch through the proxy (the `fetchTodos` server function in src/routes/index.tsx),
// tagged by registerProxyFetch() in src/router.tsx. Each test seeds a DISTINCT todo
// and asserts the server-rendered page shows only its own. Under parallel replay
// these run as concurrent proxy sessions, so the SSR fetch MUST carry the recording
// id — without it the proxy can't tell the sessions apart and these assertions fail.
