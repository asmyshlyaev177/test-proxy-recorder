...
//
//   mock backend  ──>  test-proxy-recorder (CLI, config-driven)  ──>  TanStack app
//   (own data file)    (own config file)                              (own Nitro server)
//
...
//                                        through a browser against the app
...
// The production build output (`vite build`), started like `pnpm start`.
const APP_SERVER = path.join(process.cwd(), '.output', 'server', 'index.mjs');
...
test.describe('full app e2e — every config option applied', () => {
  ...
  test.beforeAll(async () => {
    ...
    // TanStack Start app (production build) pointed at the proxy for SSR
    // (BACKEND_URL is read at runtime by the server functions). The recorder is
    // enabled explicitly since this is a production build.
    app = spawn(APP_NODE, [APP_SERVER], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        PORT: String(appPort),
        HOST: '127.0.0.1',
        BACKEND_URL: proxyUrl(),
    ...
    await waitForHttp(appUrl(), 'tanstack app');
  });
