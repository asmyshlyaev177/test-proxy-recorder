...
// Credentials for the Cognito test user. Provided via env: `.env.local` locally
// (gitignored) or CI secrets. See README. The public pool/client id, by contrast,
// is baked into the client bundle (VITE_COGNITO_*).
const EMAIL = process.env.COGNITO_TEST_EMAIL;
const PASSWORD = process.env.COGNITO_TEST_PASSWORD;

// Logs in ONCE before the authenticated specs. The proxy is put into `transparent`
// mode so nothing the login does is recorded; the Cognito call itself also goes to
// a different host than the proxy, so it never touches a recording either way. The
// resulting session (the access token in localStorage) is saved to a gitignored
// storageState that the `auth` project reuses.
//
...
setupAuth('authenticate', async ({ page }) => {
  ...
  // down (the case for the backend-less replay stack). storageState only needs the
  // token, not the data.
